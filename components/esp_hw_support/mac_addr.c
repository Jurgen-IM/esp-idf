...
{
	memset((void*)mac,0x00,ESP_MAC_ADDRESS_LEN);
...
esp_err_t esp_efuse_mac_set_custom(uint8_t *mac)
{
#if !CONFIG_IDF_TARGET_ESP32
    size_t size_bits = esp_efuse_get_field_size(ESP_EFUSE_USER_DATA_MAC_CUSTOM);
    assert((size_bits % 8) == 0);
    ...
    if (mac[0] == 0 && memcmp(mac, &mac[1], size - 1) == 0) {
        ...
        esp_efuse_write_field_blob(ESP_EFUSE_USER_DATA_MAC_CUSTOM, mac, size_bits);
    ...
    {
        esp_efuse_batch_write_begin();
        esp_efuse_write_field_blob(ESP_EFUSE_MAC_CUSTOM, mac, ESP_MAC_ADDRESS_LEN*8);
        uint8_t calc_crc = esp_rom_efuse_mac_address_crc8(mac, ESP_MAC_ADDRESS_LEN);
        esp_efuse_write_field_blob(ESP_EFUSE_MAC_CUSTOM_CRC, &calc_crc, 8);
        version = 1;
        esp_efuse_write_field_blob(ESP_EFUSE_MAC_CUSTOM_VER, &version, 8);
        esp_efuse_batch_write_commit();
    }
    else
    {
...
static esp_err_t esp_increment_mac(uint8_t *mac, int inc)
{
	for(int i=ESP_MAC_ADDRESS_LEN-1;i;i--)
	{
		uint16_t res = mac[i]+inc;
		mac[i] = res&0xff;
		inc=res>>8;
		if(!inc)
		{
...
esp_err_t esp_read_mac(uint8_t *mac, esp_mac_type_t type)
{
    ...
    case ESP_MAC_WIFI_STA:
        memcpy(mac, efuse_mac, ESP_MAC_ADDRESS_LEN);
        esp_increment_mac(mac,1);
        break;
    case ESP_MAC_WIFI_SOFTAP:
        ...
        uint8_t mac_begin[ESP_MAC_ADDRESS_LEN] = { 0x7c, 0xdf, 0xa1, 0x00, 0x30, 0x00 };
        uint8_t mac_end[ESP_MAC_ADDRESS_LEN]   = { 0x7c, 0xdf, 0xa1, 0x00, 0x5f, 0xff };
        if (memcmp(mac, mac_begin, ESP_MAC_ADDRESS_LEN) >= 0 && memcmp(mac_end, mac, ESP_MAC_ADDRESS_LEN) >= 0 ) {
            mac[ESP_MAC_ADDRESS_LEN-3] += 0x02; // contain carry bit
            mac[ESP_MAC_ADDRESS_LEN-2] += 0xd0;
        } else {
            mac[ESP_MAC_ADDRESS_LEN-1] += 1;
        }
#else
        esp_increment_mac(mac,2);
    ...
    case ESP_MAC_BT:
    	...
    	esp_efuse_mac_get_default(efuse_mac); //always eq. base mac
        memcpy(mac, efuse_mac, ESP_MAC_ADDRESS_LEN);
    ...
    case ESP_MAC_ETH:
        ...
        memcpy(mac, efuse_mac, ESP_MAC_ADDRESS_LEN);
        esp_increment_mac(mac,3);
#else
        efuse_mac[ESP_MAC_ADDRESS_LEN-1] += 1;
        esp_derive_local_mac(mac, efuse_mac);
